 ...
 * \endcode
 */
bool XmlValidator::validateCDataSection(const Common::UnicodeString &cdata)
{
    bool valid = true;
    ...
    if (!cdata.empty())
    {
        ...
        while ((position < cdata.size()) && !validationFinished)
        {
            // Check if valid attribute value character
            const uint32_t uchar = cdata.at(position);
            ...
            if (uchar == static_cast<uint32_t>('>'))
            {
                ...
                {
                    // Check for ']]>' sequence
                    if (Common::compareUnicodeString(position - 2U, cdata, "]]"))
                    {
